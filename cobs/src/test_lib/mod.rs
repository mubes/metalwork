#[cfg(test)]
use super::*;

#[test]
fn test_setting_sentinel() {
    let mut dec = Cobs::new();
    assert_eq!(dec.sentinel, 0);
    dec.set_sentinel(45, true).unwrap();
    assert_eq!(dec.sentinel, 45);
}

/// The set of test cases from https://en.wikipedia.org/wiki/Consistent_Overhead_Byte_Stuffing
#[test]
fn wikipedia_tests() {
    struct Test {
        result: Vec<u8>,
        input: Vec<u8>,
    }

    let tests = vec![
        Test {
            result: vec![0x00u8],
            input: vec![1u8, 1, 0],
        },
        Test {
            result: vec![0x00u8, 0],
            input: vec![1u8, 1, 1, 0],
        },
        Test {
            result: vec![0x00, 0x11, 0x00],
            input: vec![0x01, 0x02, 0x11, 0x01, 0x00],
        },
        Test {
            result: vec![0x11, 0x22, 0x00, 0x33],
            input: vec![0x03, 0x11, 0x22, 0x02, 0x33, 0x00],
        },
        Test {
            result: vec![0x11, 0x22, 0x33, 0x44],
            input: vec![0x05, 0x11, 0x22, 0x33, 0x44, 0x00],
        },
        Test {
            result: vec![0x11, 0x00, 0x00, 0x00],
            input: vec![0x02, 0x11, 0x01, 0x01, 0x01, 0x00],
        },
        Test {
            result: vec![
                ,
            ],
            input: vec![
                ,
            ],
        },
        Test {
            result: vec![
                ,
            ],
            input: vec![
                ,
            ],
        },
        Test {
            result: vec![
                ,
            ],
            input: vec![
                ,
            ],
        },
        Test {
            result: vec![
                ,
            ],
            input: vec![
                ,
            ],
        },
        Test {
            result: vec![
                ,
            ],
            input: vec![
                ,
            ],
        },
    ];

    println!("Check wikipedia test cases");
    for n in 0..tests.len() {
        let mut dec: Cobs = Cobs::new();
        //let mut v = Vec::<u8>::with_capacity(10000);
        let v = dec.get_frame_as_vec(tests[n].input.iter()).unwrap();

        assert!(v == tests[n].result);
        println!(
            "Example {}: {}",
            n + 1,
            if v == tests[n].result { "OK" } else { "Failed" }
        );

        assert_eq!(
            dec.stats(),
            (
                tests[n].input.len() as u64,
                tests[n].result.len() as u64,
                0,
                1,
                0
            )
        );
    }
}

#[test]
#[allow(dead_code)]
fn check_incomplete_packet() {
    struct Test {
        input: Vec<u8>,
        result: Vec<u8>,
    }

    const BUFFER_CAP: usize = 150;

    let test_packet = Test {
        result: vec![
            ,
        ],
        input: vec![
            ,
        ],
    };

    let mut dec: Cobs = Cobs::new();
    let mut v = Vec::<u8>::with_capacity(BUFFER_CAP);
    let v = dec.get_frame(test_packet.input.iter(), &mut v);
    println!("{:?}", v);
    assert_eq!(
        dec.stats(),
        (
            test_packet.input.len() as u64,
            0,
            test_packet.result.len() as u64 - 1,
            0,
            1
        )
    );
}

#[test]
fn short_packet() {
    struct Test {
        input: Vec<u8>,
        result: Vec<u8>,
    }

    let noisetest = Test {
        result: vec![0x11, 0x22, 0x33, 0x44],
        input: vec![
            0x05, 0x00, 0x22, 0x33, 0x44, 0x00, 0x05, 0x11, 0x22, 0x33, 0x44, 0x00,
        ],
    };

    let mut dec: Cobs = Cobs::new();
    let mut v = Vec::<u8>::with_capacity(150);
    let _ = dec.get_frame(noisetest.input.iter(), &mut v);

    assert_eq!(dec.stats(), (12, 4, 3, 1, 0));
    assert_eq!(noisetest.result, v);
}

#[test]
fn byte_feeder() {
    let result = vec![
        0x03u8, ,
    ];

    let input = vec![
        0xfeu8, ,
    ];

    let mut dec = Cobs::new();
    let mut v = Vec::<u8>::with_capacity(10000);
    for p in &input {
        let _ = dec.get_byte(*p, &mut v);
    }
    assert!(v == result);
    assert_eq!(
        dec.stats(),
        (input.len() as u64, result.len() as u64, 0, 1, 0)
    );
}

#[test]
fn test_encode() {
    struct Test {
        result: Vec<u8>,
        input: Vec<u8>,
    }

    let tests = vec![
        Test {
            result: vec![0x00u8],
            input: vec![1u8, 1, 0],
        },
        Test {
            result: vec![0x00u8, 0],
            input: vec![1u8, 1, 1, 0],
        },
        Test {
            result: vec![0x00, 0x11, 0x00],
            input: vec![0x01, 0x02, 0x11, 0x01, 0x00],
        },
        Test {
            result: vec![0x11, 0x22, 0x00, 0x33],
            input: vec![0x03, 0x11, 0x22, 0x02, 0x33, 0x00],
        },
        Test {
            result: vec![0x11, 0x22, 0x33, 0x44],
            input: vec![0x05, 0x11, 0x22, 0x33, 0x44, 0x00],
        },
        Test {
            result: vec![0x11, 0x00, 0x00, 0x00],
            input: vec![0x02, 0x11, 0x01, 0x01, 0x01, 0x00],
        },
        Test {
            // 6
            result: vec![
                ,
            ],
            input: vec![
                ,
            ],
        },
        Test {
            // 7
            result: vec![
                ,
            ],
            input: vec![
                ,
            ],
        },
        Test {
            // 8
            result: vec![
                ,
            ],
            input: vec![
                ,
            ],
        },
        Test {
            result: vec![
                ,
            ],
            input: vec![
                ,
            ],
        },
        Test {
            result: vec![
                ,
            ],
            input: vec![
                ,
            ],
        },
    ];

    for t in 0..tests.len() {
        let dec = Cobs::new();
        let r = dec.cobs_encode_into_vec(&vec![&tests[t].result]).unwrap();
        assert_eq!(r, tests[t].input);
        println!("Test {}: OK", t + 1);
    }
}

#[test]
fn short_test() {
    let c = Cobs::new();
    let encver = c.cobs_encode_into_vec(&vec![&vec![0u8; 0]]);
    assert_eq!(encver, Err(CobsError::ZeroLength));
}

#[test]
fn smoke_test() {
    print! {"Smoke test for 10000 random frames"};
    //let mut rng = rand::thread_rng();
    for _n in 1..10000 {
        let original: Vec<u8> = (1..2 + fastrand::usize(2..8192))
            .map(|_| fastrand::u8(0..255))
            .collect();
        let mut c = Cobs::new();
        let encver = c.cobs_encode_into_vec(&vec![&original]).unwrap();
        let mut dec_candidate = Vec::<u8>::with_capacity(8192);
        let _ = c.get_frame(encver.iter(), &mut dec_candidate);

        for (a, _b) in original.iter().enumerate() {
            if original[a] != dec_candidate[a] {
                println!(
                    "index={} original={} candidate={} len={}",
                    a,
                    original[a],
                    dec_candidate[a],
                    original.len()
                );
                println!(
                    "Original len={}, Cand len={}",
                    original.len(),
                    dec_candidate.len()
                );
                println!("ORIGINAL={:?}", original);
                println!("ENCODED={:?}", encver);
                println!("CANDIDATE={:?}", dec_candidate);
                panic!();
            }
        }

        assert_eq!(original, dec_candidate);
    }
}
